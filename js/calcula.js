...
function seno() {
  x = Math.sin(x * PI / 180)//saca sen
  pantalla.innerHTML = x; //muestra en pantalla
  op = "no";//quita operaciones pendientes
  xi = 1;//se reinicia la pantalla
}
function cos() {
  x = Math.cos(x * PI / 180)
  pantalla.innerHTML = x; //mostrar resultado en pantalla
  op = "no";
  xi = 1;//reinicio de pantalla
}
function Tan() {
  x = Math.tan(x * PI / 180)
...
function bin(){
  num = parseInt(this.x, 10)
  binario = "";
  binario = num.toString("2");
  pantalla.innerHTML = binario;
  op = "no";
  xi = 1;
}
